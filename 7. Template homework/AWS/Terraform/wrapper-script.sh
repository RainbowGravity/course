...
echo -e "This is a wrapper script for Terraform template homework. 
...
CheckZones(){
    ...
    do
        ...
        if [ $? -eq 0 ] && [ $Amount_of_Zones -le $zones ] 2>/dev/null && [ $Amount_of_Zones -ge 2 ] 2>/dev/null
            then 
                check=$"1"
...
CheckInstances(){
    ...
    do
        ...
        if [ $? -eq 0 ] && [ $EC2_Per_Zone -ge 1 ] 2>/dev/null
            then 
                check=$"1"
...
WriteChanges(){
    echo -e "\n\033[0;33mWriting changes to varscript.tfvars...\033[0m"
    now=$(date +"%d""-%m""-%Y"" %T")
    echo -e "#===========================================================================================
# Rainbow Gravity's Template homework
# 
# Created by a script on $now
#===========================================================================================
" > varscript.tfvars
    echo 'Region          = "'$Region'"'         >> varscript.tfvars
...
EC2 Instance type|\033[0;32m$Instance_Type\033[0m
...
echo -e "\n\033[0;33mWrite this changes to varscript.tfvars?\033[0m

Only 'yes' will be accepted to approve."
read changes
changes=$(echo $changes | awk '{print tolower($0)}')
if [ $changes == "yes" ]
    then 
        WriteChanges
    else
        echo -e "\nChanges cancelled. Exiting."
        exit
fi
# Starting 'terraform plan' with variables which was specified during script running
echo -e "\n\033[0;33mWriting a plan...\033[0m"
