...
# 
# Instances SSM Role and Instance S3 Read Role
#----------------------------------------------------------------------
...
resource "aws_iam_instance_profile" "Instances_SSM_Profile" {
  name = "${var.Tags["Environment"]}InstancesSSMProfile"
  role = aws_iam_role.Instances_SSM_Role.name
}

resource "aws_iam_role" "Instances_SSM_Role" {
  name = "${var.Tags["Environment"]}InstancesSSMRole"
  ...
  tags = merge(var.Tags, { Name = "${var.Tags["Environment"]}InstancesSSMRole" })
}

resource "aws_iam_role_policy" "Instances_SSM_Policy" {
  name = "${var.Tags["Environment"]}InstancesSSMPolicy"
