...
* Script can take and process even only the username! For example, you can enter the ```spring-projects``` and you will get the list of repositories with stars and discriptions. But only 100 of the latest repositories will be displayed.
...
* <b>-s</b> - amount of open PRs to process per page. Can be used like in the previous example. ```-c10``` for 10 results or ```-cn``` to skip questions and use the default amount (30). Can't be more than 100.
...
* <b>-r</b> - by this option you can set the link or username to skip questions about it during the script running. Example: ```-r https://github.com/foo/bar``` or ```-r foo/bar``` or ```-r foo```.
